...
keywords = ['hello', 'roll', 'add', 'div', 'mul','sub']
...
def handle_response(message) -> str:
    ...
    if p_message.startswith(keywords[4]):
        current_number = ''
        result = None
        is_in_number = False

        for character in p_message[len(keywords[4]):]:
            if character.isdigit():
                current_number += character
                is_in_number = True
    ...
    if p_message.startswith(keywords[5]):
        current_number = ''
        result = None
        is_in_number = False

        for character in p_message[len(keywords[5]):]:
            if character.isdigit():
                current_number += character
                is_in_number = True
    ...
    if p_message == 'help':
        ...
        return f'The keywords are "{keywords1}"' \
               ...
               f'\n{keywords[4]}: Will Multiply any numbers included in the message' \
               f'\n{keywords[5]}: Will subtract any numbers included in the message'